...
public class USBHIDReader {
	...
	private final int vendorID = 0x6666;
	private final int productID = 0x7777;
	...
	private boolean enumerateDevice() {
		...
		while (deviceIterator.hasNext()) {
			...
			Log.i(TAG, "DeviceInfo: " + Integer.toHexString(device.getVendorId())  + " , "
					+ Integer.toHexString(device.getProductId())  +",getProductName:"+device.getProductName());
			if (device.getVendorId() == vendorID && device.getProductId() == productID) {
				Log.i(TAG, "found itDeviceInfo a1 : " + Integer.toHexString(device.getVendorId())  + " , "
						+ Integer.toHexString(device.getProductId())  +",getProductName:"+device.getProductName());
				if(usbManager.hasPermission(device)) {
					usbDevice = device;
					if(findInterface()) {
						if(assignEndpoint()) {
							Log.i(TAG, "found itDeviceInfo a2: " + Integer.toHexString(device.getVendorId())  + " , "
									+ Integer.toHexString(device.getProductId())  +",getProductName:"+device.getProductName());
							return openDevice();
						}
						else {
							Log.i(TAG, "found itDeviceInfo a2. 2 : ");
						}
					}
					else
					{
...
//						&& intf.getInterfaceProtocol() == 80) {
				...
				/*else if (intf.getInterfaceClass() == UsbConstants.USB_CLASS_COMM) {

					usbHidInterface = intf;
				}*/
				else if (intf.getInterfaceClass() == UsbConstants.USB_CLASS_CDC_DATA) {

					usbHidInterface = intf;
				}
				else
				{
					Log.d(TAG, "interfaceCounts  this one is not USB_CLASS_HID : " + intf.getInterfaceClass());
				}
			}
		return true;
	}
